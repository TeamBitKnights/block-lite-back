...
const dbChartData = async (req, res) => {
  try {
    ...
    const getChartData =
      'SELECT * FROM transaction_data WHERE LOCATE( ? , time_stamp) > 0 ;';
...
const dbLatestBlockData = async (req, res) => {
  try {
    const dbLatestBlockData =
      'SELECT * FROM block_data ORDER BY blocknumber DESC LIMIT 10;';
...
const dbAllBlocks = async (req, res) => {
  try {
    const dbAllBlocks =
      'SELECT MAX(blocknumber) AS max_number FROM block_data;';
    db.query(dbAllBlocks, (err, result) => {
      if (!req.query.page) {
        const firstPageBlock =
          'SELECT * FROM block_data ORDER BY blocknumber DESC LIMIT 50;';
        ...
        let startIndex =
          allBlocks - countPerPage * (pageNo - 1) - countPerPage + 1;
        ...
        console.log(
          countPerPage,
          '개씩 총 페이지 수',
          Math.ceil(allBlocks / countPerPage)
        );
        ...
        const pageBlock =
          'SELECT * FROM block_data WHERE blocknumber BETWEEN ? and ?;';
...
const dbLatestTxs = async (req, res) => {
  try {
    const dbLatestTxsData =
      'SELECT * FROM transaction_data ORDER BY blockNumber DESC LIMIT 10;';
...
const dbAllTxs = async (req, res) => {
  try {
    ...
    db.query(dbAllTxs, (err, result) => {
      if (!req.query.page) {
        const firstPageBlock =
          'SELECT * FROM transaction_data ORDER BY blockNumber DESC LIMIT 50;';
        db.query(firstPageBlock, (err, result) => {
          res.json(result);
        });
      } else {
        ...
        const pageTx =
          'SELECT * FROM transaction_data ORDER BY blockNumber DESC LIMIT ?, ?;';
...
const dbBlockTxs = async (req, res) => {
  try {
    ...
    const getBlockTxs =
      'SELECT * FROM transaction_data WHERE blockNumberHex = ?;';
...
const dbAddressTxs = async (req, res) => {
  try {
    ...
    if (!req.query.page) {
      const getAddressTxs =
        'SELECT * FROM transaction_data WHERE fromAddress = ? OR toAddress = ? ORDER BY time_stamp DESC LIMIT 25;';
      ...
      const getAddressTxs =
        'SELECT * FROM transaction_data WHERE fromAddress = ? OR toAddress = ?';
      db.query(getAddressTxs, [Address, Address], (err, result) => {
        ...
        console.log(
          countPerPage,
          '개씩 총 페이지 수',
          Math.ceil(allTxsNum / countPerPage)
        );
        ...
        const pageTx =
          'SELECT * FROM transaction_data WHERE fromAddress = ? OR toAddress = ? ORDER BY time_stamp DESC LIMIT ?, ?;';
        db.query(
          pageTx,
          [Address, Address, startIndex, countPerPage],
          (err, result) => {
            console.log(result);
            res.json(result);
          }
...
const dbAddressTxsNum = async (req, res) => {
  try {
    ...
    const getAddressTxs =
      'SELECT * FROM transaction_data WHERE fromAddress = ? OR toAddress = ? ORDER BY time_stamp;';
    db.query(getAddressTxs, [Address, Address], (err, result) => {
      //console.log(err)
...
const dbAddressCheck = async (req, res) => {
  try {
    ...
    const getAddressCheck =
      "SELECT IF(EXISTS(SELECT * from contract_data  WHERE contractAddress = ? ), '1', '0' ) as RESULT;";
...
const chartAllTxsByDate = async (req, res) => {
  try {
    const chartData =
      "SELECT DATE(`time_stamp`) AS 'day', COUNT(*) AS 'txLength' FROM `transaction_data`GROUP BY DATE(`time_stamp`);";
    db.query(chartData, (err, result) => {
      ...
      let stDate = new Date(2024, 09, 20);
